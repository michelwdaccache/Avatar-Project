...
connectButton.onclick = async () => {
  ...
  const sessionResponse = await fetchWithRetries(`${DID_API.url}/${DID_API.service}/streams`, {
    ...
    body: JSON.stringify({
      source_url: 'https://res.cloudinary.com/dxlwe7rrm/image/upload/v1718965409/d-id/Image_aksntk.png',
    }),
  });
...
function playIdleVideo() {
  idleVideoElement.src = 'https://res.cloudinary.com/dxlwe7rrm/image/upload/v1718965409/d-id/Image_aksntk.png';
...
sendBtn.addEventListener('click', async (e) => {
  ...
  response.textContent = result.query_response;
  ...
  if (
    ...
    const playResponse = await fetchWithRetries(`${DID_API.url}/${DID_API.service}/streams/${streamId}`, {
      ...
      headers: {
        Authorization: `Basic ${DID_API.key}`,
        'Content-Type': 'application/json',
        'x-api-key-external': '{"elevenlabs": "sk_25dda9282930e22f7e91219ff58a13c71965c93629632cbd"}'
      },
      body: JSON.stringify({
        script: {
          type: 'text',
          ...
          provider: { type: 'elevenlabs', voice_id: '12zihrNpodm7mJIrCw7b' },
          ssml: true,
          input: result.query_response, // Use the user input as the input value
        },
        config: {
          fluent: true,
          pad_audio: 0,
  ...
  if (
    ...
    const playResponse = await fetchWithRetries(`${DID_API.url}/${DID_API.service}/streams/${streamId}`, {
      ...
      headers: {
        Authorization: `Basic ${DID_API.key}`,
        'Content-Type': 'application/json',
        'x-api-key-exteranl': '{"elevenlabs": "sk_25dda9282930e22f7e91219ff58a13c71965c93629632cbd"}'
      },
      body: JSON.stringify({
        script: {
          type: 'text',
